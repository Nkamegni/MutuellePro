// Service worker — espace-client
// Cache nommé "client-*" : garde-fou explicite contre toute fuite entre
// rôles via la Cache Storage API (partagée par origine, pas par scope —
// voir document d'architecture, section 2).
const CACHE_NAME = 'client-shell-v1';
const APP_SHELL = [
  '/espace-client/',
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((noms) =>
      Promise.all(noms
        .filter((nom) => nom.startsWith('client-') && nom !== CACHE_NAME)
