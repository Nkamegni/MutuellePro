...
module.exports = function (pool) {
    ...
    router.post('/tickets', async (req, res) => {
        ...
        //
        // Préfixe par type (DDEV/DSIN/DINFO, colonne site.type_ticket.prefixe_code,
        // 21/08/2026) — remplace l'ancien préfixe générique "TCK-", qui ne
        // distinguait pas le type de demande. Format : PREFIXE-AAMMJJ-NNNN,
        // numérotation centrale (id_ticket), pas de compteur séparé par type.
        const requete = `
            WITH nextid AS (
            ...
            SELECT
                nextid.id_ticket,
                tt.id_type_ticket,
                tt.prefixe_code || '-' || to_char(now(), 'YYMMDD') || '-' || lpad(nextid.id_ticket::text, 4, '0'),
                $2::jsonb, $3, $4
