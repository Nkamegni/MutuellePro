...
const ticketsRateLimiter = createRateLimiter(10, 10 * 60 * 1000); // Ajoute le 27/08/2026 -- protection de POST /api/tickets, absente depuis sa creation (signale 22/08, confirme absent 26/08).
...
app.use('/api', (req, res, next) => {
    if (req.method === 'POST' && req.path === '/tickets') {
        return ticketsRateLimiter(req, res, next);
    }
    next();
});
...
const staffSession = session({
    name: 'connect.sid.staff',
    store: new pgSession({ pool, schemaName: 'site', tableName: 'session_staff' }),
...
app.use('/api/staff', staffSession);
...
const monProfilStaffRouter = require('./routes/monProfilStaff.routes')(pool);
app.use('/api/staff', monProfilStaffRouter);
const staffPersonnelRouter = require('./routes/staffPersonnel.routes')(pool);
app.use('/api/staff', staffPersonnelRouter);
const activationStaffRouter = require('./routes/activationStaff.routes')(pool);
...
const partenaireSession = session({
    ...
    secret: process.env.PARTENAIRE_SESSION_SECRET,
    resave: false,
    saveUninitialized: false,
    rolling: true,
    cookie: { httpOnly: true, secure: true, sameSite: 'strict', maxAge: 30 * 24 * 60 * 60 * 1000 }
});
app.use('/api/partenaire', partenaireSession);
