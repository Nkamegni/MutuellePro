// Service worker — espace-personnel
// Cache nommé "personnel-*" : garde-fou explicite contre toute fuite entre
// rôles via la Cache Storage API (partagée par origine, pas par scope —
// voir document d'architecture, section 2).
const CACHE_NAME = 'personnel-shell-v1';
const APP_SHELL = [
  '/espace-personnel/',
  '/espace-personnel/index.html',
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((noms) =>
      Promise.all(noms
        .filter((nom) => nom.startsWith('personnel-') && nom !== CACHE_NAME)
